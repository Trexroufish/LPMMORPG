...
public partial class Player : Entity {
        ...
        if ((state == "IDLE" || state == "MOVING" || state == "CASTING") &&
            ...
            if (item.category.StartsWith("Potion")) {
                ...
                if (item.petPrefab != null) {
                    // summon: only if no other is summoned yet and if pet not dead
                    // (dead pet item has to be revived first)
                    if (activePet == null && item.petHealth > 0) {
    ...
    void OnEquipmentChanged(SyncListItem.Operation op, int index) {
        // update the model
        Debug.LogWarning("OnEquipmentChanged " + name);
